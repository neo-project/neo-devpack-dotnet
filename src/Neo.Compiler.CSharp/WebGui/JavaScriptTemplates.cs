...
//
// JavaScriptTemplates.cs file belongs to the neo project and is free
...
{
    /// <summary>
    /// JavaScript templates for web GUI generation
    /// </summary>
    internal static class JavaScriptTemplates
    {
        public static string GetMainScript(string contractName, ContractManifest manifest, WebGuiOptions options)
        {
            ...
            js.AppendLine(GetConfigurationScript(contractName, manifest, options));

            // Core functionality
            js.AppendLine(GetCoreScript());

            // Tab management
            js.AppendLine(GetTabManagementScript());

            // RPC communication
            js.AppendLine(GetRpcScript());

            if (options.IncludeMethodInvocation)
                js.AppendLine(GetMethodInvocationScript(manifest));

            if (options.IncludeBalanceMonitoring)
                js.AppendLine(GetBalanceMonitoringScript());

            if (options.IncludeTransactionHistory)
                js.AppendLine(GetTransactionHistoryScript());

            if (options.IncludeEventMonitoring)
                js.AppendLine(GetEventMonitoringScript());

            if (options.IncludeStateMonitoring)
                js.AppendLine(GetStateMonitoringScript());

            if (options.IncludeWalletConnection)
                js.AppendLine(GetWalletConnectionScript());

            // Theme management
            js.AppendLine(GetThemeScript());

            // Auto-refresh
            if (options.RefreshInterval > 0)
                js.AppendLine(GetAutoRefreshScript(options.RefreshInterval));

            // Initialization
            js.AppendLine(GetInitializationScript());

            return js.ToString();
        }

        private static string GetConfigurationScript(string contractName, ContractManifest manifest, WebGuiOptions options)
        {
            var methods = manifest.Abi.Methods.Select(m => new
            {
                name = m.Name,
                parameters = m.Parameters.Select(p => new { name = p.Name, type = p.Type.ToString() }).ToArray(),
                returnType = m.ReturnType.ToString(),
                safe = m.Safe
            }).ToArray();

            var events = manifest.Abi.Events.Select(e => new
            {
                name = e.Name,
                parameters = e.Parameters.Select(p => new { name = p.Name, type = p.Type.ToString() }).ToArray()
            }).ToArray();

            var config = new
            {
                contract = new { name = contractName, methods, events },
                network = new { rpcEndpoint = options.RpcEndpoint, magic = options.NetworkMagic },
                features = new
                {
                ...
                ui = new { refreshInterval = options.RefreshInterval, darkTheme = options.DarkTheme }
            };
...
";
        ...
        private static string GetTabManagementScript()
        {
            return @"
// Tab management
const tabManager = {
...
";
        ...
        private static string GetRpcScript()
        {
            return @"
// RPC communication
const rpc = {
...
";
        ...
        private static string GetMethodInvocationScript(ContractManifest manifest)
        {
            var methodsJson = JsonSerializer.Serialize(manifest.Abi.Methods.Select(m => new
            {
                name = m.Name,
                parameters = m.Parameters.Select(p => new { name = p.Name, type = p.Type.ToString() }).ToArray(),
                returnType = m.ReturnType.ToString(),
...
";
        ...
        private static string GetBalanceMonitoringScript()
        {
            return @"
// Balance monitoring
...
";
        ...
        private static string GetTransactionHistoryScript()
        {
            return @"
// Transaction history
...
";
        ...
        private static string GetEventMonitoringScript()
        {
            return @"
// Event monitoring
const eventMonitor = {
...
";
        ...
        private static string GetStateMonitoringScript()
        {
            return @"
// State monitoring
const stateMonitor = {
...
";
        ...
        private static string GetWalletConnectionScript()
        {
            return @"
// Wallet connection
const walletManager = {
...
";
        ...
        private static string GetThemeScript()
        {
            return @"
// Theme management
const themeManager = {
...
";
        ...
        private static string GetAutoRefreshScript(int intervalSeconds)
        {
            return $@"
