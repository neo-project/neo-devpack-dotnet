...
namespace Neo.Compiler.SecurityAnalyzer
{
    ...
    {
        private const string ReentrancyGuardsExtraKey = "reentrancyGuards";
            ...
            (NefFile nef, ContractManifest manifest, JToken? debugInfo = null)
        {
            ...
            // Remove blocks that belong to methods explicitly guarded against re-entrancy.
            var guardedBlocks = CollectGuardedBlocks(contractInBasicBlocks, manifest);
            if (guardedBlocks.Count > 0)
            {
                foreach (var block in guardedBlocks)
                {
                    vulnerabilityPairs.Remove(block);
                    callOtherContractInstructions.Remove(block);
                    writeStorageInstructions.Remove(block);
                }

                foreach (var kv in vulnerabilityPairs.ToList())
                    kv.Value.RemoveWhere(guardedBlocks.Contains);
            }
            return new(vulnerabilityPairs, callOtherContractInstructions, writeStorageInstructions, debugInfo);
        }

        private static HashSet<int> GetGuardedMethodOffsets(ContractManifest manifest)
        {
            HashSet<int> offsets = new();
            if (manifest.Extra is JObject extra && extra.ContainsProperty(ReentrancyGuardsExtraKey))
            {
                if (extra[ReentrancyGuardsExtraKey] is JObject guards)
                {
                    foreach (var method in manifest.Abi.Methods)
                    {
                        if (guards.ContainsProperty(method.Name))
        ...
        private static HashSet<BasicBlock> CollectGuardedBlocks(ContractInBasicBlocks context, ContractManifest manifest)
        {
            HashSet<BasicBlock> guarded = new();
            foreach (var offset in GetGuardedMethodOffsets(manifest))
            {
                if (!context.basicBlocksByStartAddr.TryGetValue(offset, out var entryBlock))
                    continue;

                foreach (var block in context.BlocksCoveredFromAddr(offset))
                    guarded.Add(block);
            }
