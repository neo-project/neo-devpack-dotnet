...
//
// ConditionalAccessExpression.cs file belongs to the neo project and is free
...
internal partial class MethodConvert
{
    ...
    {
        if (expression.Expression is IdentifierNameSyntax eventIdentifier &&
            model.GetSymbolInfo(eventIdentifier).Symbol is IEventSymbol eventSymbol &&
            expression.WhenNotNull is InvocationExpressionSyntax invocation &&
        ...
        {
            ConvertEventInvocationExpression(model, eventSymbol, invocation.ArgumentList.Arguments.ToArray());
            return;
        }
