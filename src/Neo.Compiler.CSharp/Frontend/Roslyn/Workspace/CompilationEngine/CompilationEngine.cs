...
namespace Neo.Compiler
{
    public class CompilationEngine(CompilationOptions options)
    {
        ...
        {
            Options.Logger?.Invoke($"[{DateTime.UtcNow:HH:mm:ss.fff}] {message}");
        }
        ...
        {
            Log($"CompileSources invoked with {sourceFiles.Length} file(s).");
...
<configuration>
  <packageSources>
    ...
    <add key=""NuGet.org"" value=""https://api.nuget.org/v3/index.json"" protocolVersion=""3"" />
            ...
            Log($"CompileSources: temporary project created at {path}");

            try
            {
                Log("CompileSources: invoking CompileProject for temporary project.");
                return CompileProject(path);
            }
            finally
            {
                Log("CompileSources: cleaning up temporary project directory.");
        ...
        {
            Log($"CompileProject invoked for {csproj}");
        ...
        {
            Log($"PrepareProjectContracts start for {csproj}");
            Compilation ??= GetCompilation(csproj);
            ...
            {
                Log($"PrepareProjectContracts: processing syntax tree {tree.FilePath ?? "<generated>"}");
            ...
            if (classDependencies.Count == 0)
                throw new FormatException("No valid neo SmartContract found. Please make sure your contract is subclass of SmartContract and is not abstract.");
            ...
            Log($"PrepareProjectContracts completed. Sorted contracts: {string.Join(", ", sortedClasses.Select(c => c.Name))}");
        ...
        private CompilationContext CompileProjectContractWithPrepare(List<INamedTypeSymbol> sortedClasses, Dictionary<INamedTypeSymbol, List<INamedTypeSymbol>> classDependencies, List<INamedTypeSymbol?> allClassSymbols, string targetContractName)
        {
            ...
            Log($"CompileProjectContractWithPrepare: compiling target contract {c.Name}");
            ...
            context.Compile();
            Log($"CompileProjectContractWithPrepare: completed contract {c.Name}");
            return context;
        }
        ...
        private List<CompilationContext> CompileProjectContractsWithPrepare(List<INamedTypeSymbol> sortedClasses, Dictionary<INamedTypeSymbol, List<INamedTypeSymbol>> classDependencies, List<INamedTypeSymbol?> allClassSymbols)
        {
            ...
            Log($"CompileProjectContractsWithPrepare: scheduling {sortedClasses.Count} contract(s).");
            Parallel.ForEach(sortedClasses, c =>
            {
                Log($"[Thread {Thread.CurrentThread.ManagedThreadId}] Begin compile for contract {c.Name}");
                var dependencies = classDependencies.TryGetValue(c, out var dependency) ? dependency : [];
                var classesNotInDependencies = allClassSymbols.Except(dependencies).ToList();
                var context = new CompilationContext(this, c, classesNotInDependencies!, allowBaseName);
                context.Compile();
                Log($"[Thread {Thread.CurrentThread.ManagedThreadId}] Completed compile for contract {c.Name}");
                // Process the target contract add this compilation context
                Contexts.TryAdd(c, context);
            });
            Log("CompileProjectContractsWithPrepare: all contracts compiled.");
        ...
        private List<CompilationContext> CompileProjectContracts(Compilation compilation)
        {
            ...
            foreach (var tree in compilation.SyntaxTrees)
            {
                Log($"CompileProjectContracts: processing syntax tree {tree.FilePath ?? "<generated>"}");
                var semanticModel = compilation.GetSemanticModel(tree);
                var classNodes = tree.GetRoot().DescendantNodes().OfType<ClassDeclarationSyntax>();
            ...
            // Check contract dependencies, make sure there is no cycle in the dependency graph
            var sortedClasses = TopologicalSort(classDependencies);
            Log($"CompileProjectContracts: sorted {sortedClasses.Count} contract(s): {string.Join(", ", sortedClasses.Select(c => c.Name))}");
            ...
            Log("CompileProjectContracts: all contracts compiled.");
            return Contexts.Select(p => p.Value).ToList();
        }
        ...
        internal static bool IsDerivedFromSmartContract(INamedTypeSymbol classSymbol)
        {
            ...
            while (baseType != null)
            {
                if (baseType.Name == "SmartContract" &&
                    baseType.ContainingNamespace?.ToDisplayString() == "Neo.SmartContract.Framework")
                {
                    return true;
                }
