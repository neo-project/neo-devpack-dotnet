...
namespace Neo.Compiler
{
    public class CompilationContext
    {
        ...
        private readonly JObject _reentrancyGuards = new();
        ...
        {
            _engine.Options.Logger?.Invoke($"[{DateTime.UtcNow:HH:mm:ss.fff}] [{_targetContract?.Name}] {message}");
        }
        ...
        internal HirModule? HirModule { get; }
        internal MirModule? MirModule { get; }
        internal LirModule? LirModule { get; }
        ...
        internal CompilationContext(CompilationEngine engine, INamedTypeSymbol targetContract, System.Collections.Generic.List<INamedTypeSymbol>? nonDependencies = null, bool allowBaseName = true)
        {
            ...
            if (Options.EnableHir)
            {
                HirModule = new HirModule();
                MirModule = new MirModule();
                LirModule = new LirModule();
            }
        }
        ...
        internal void Compile()
        {
            ...
            {
                Log($"Visiting syntax tree {tree.FilePath ?? "<generated>"}");
                ...
                try
                {
                    ...
                    Log($"Completed syntax tree {tree.FilePath ?? "<generated>"}");
                }
            ...
            if (Success)
            {
                Log("Compilation succeeded, performing post-processing.");
            ...
            {
                Log("Compilation finished with errors.");
            }
        ...
        public (NefFile nef, ContractManifest manifest, JObject debugInfo) CreateResults(string folder = "")
        {
            ...
            if (Options.EnableHir && HirModule is not null && MirModule is not null)
            {
                var inlineHints = new JObject();
                foreach (var (name, function) in MirModule.Functions)
                {
                    if (function.AggressiveInlineHint)
                    {
                        inlineHints[name] = true;
                    }
                }

                if (inlineHints.Children.Count > 0)
                    _manifestExtra["inlineHints"] = inlineHints;
            }
...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
        ...
        public ContractManifest CreateManifest()
        {
            ...
            if (_reentrancyGuards.Children.Count > 0)
                _manifestExtra["reentrancyGuards"] = _reentrancyGuards;

            var inlineHints = new JObject();
            foreach (var method in _methodsExported)
            {
                if (method.AggressiveInline)
                    inlineHints[method.Name] = true;
            }
            if (inlineHints.Children.Count > 0)
                _manifestExtra["inlineHints"] = inlineHints;
        ...
        private void ProcessMemberDeclaration(HashSet<INamedTypeSymbol> processed, SemanticModel model, MemberDeclarationSyntax syntax)
        {
            switch (syntax)
            {
                case BaseNamespaceDeclarationSyntax @namespace:
                    Log($"Entering namespace '{@namespace.Name}'");
                    foreach (MemberDeclarationSyntax member in @namespace.Members)
                        ProcessMemberDeclaration(processed, model, member);
                    Log($"Leaving namespace '{@namespace.Name}'");
                    break;
                case ClassDeclarationSyntax @class:
                    INamedTypeSymbol symbol = model.GetDeclaredSymbol(@class)!;
                    Log($"Discovered class {symbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}");
                    if (symbol.Name != _targetContract.Name && _nonDependencies != null && _nonDependencies.Contains(symbol))
                    {
                        Log($"Skipping class {symbol.Name} because it is outside dependency closure.");
                        return;
                    }
                    if (processed.Add(symbol))
                    {
                        ProcessClass(model, symbol);
                    }
                    else
                    {
                        Log($"Class {symbol.Name} already processed; skipping.");
                    }
                    break;
            }
        }
        ...
        {
            Log($"ProcessClass start: {symbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}");
            ...
            if (isSmartContract)
            {
                ...
                {
                    Log($"Skipping smart contract {symbol.Name} because target contract is {_targetContract.Name}.");
                    return;
                }
            ...
            Log($"ProcessClass completed: {symbol.Name}");
        ...
        {
            Log($"ProcessMethod start: {symbol.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat)} export={export}");
            ...
            Log($"ProcessMethod convert complete: {symbol.Name}");

            if (export)
            {
                var hirSignature = TryGetHirSignature(symbol);
                AbiMethod method = new(symbol, hirSignature);
                if (_methodsExported.Any(u => u.Name == method.Name && u.Parameters.Length == method.Parameters.Length))
                    throw new CompilationException(symbol, DiagnosticId.MethodNameConflict, $"Duplicate method key: {method.Name},{method.Parameters.Length}.");
                _methodsExported.Add(method);

                if (method.NoReentrant is { } guard)
                {
                    var guardDescriptor = new JObject
                    {
                        ["prefix"] = guard.Prefix,
                        ["key"] = guard.Key
                    };
                    _reentrancyGuards[method.Name] = guardDescriptor;
                }
            }
            if (export && MethodConvert.NeedInstanceConstructor(symbol))
            {
                ...
                Log($"ProcessMethod emitted forwarder for {symbol.Name}");
            }
            Log($"ProcessMethod completed: {symbol.Name}");
        }

        internal HirSignature? TryGetHirSignature(IMethodSymbol symbol)
        {
            if (!Options.EnableHir || HirModule is null)
                return null;

            var name = symbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
            return HirModule.TryGetFunction(name, out var function) ? function.Signature : null;
        }
        ...
        internal MethodConvert ConvertMethod(SemanticModel model, IMethodSymbol symbol)
        {
            ...
            {
                Log($"ConvertMethod cache miss: {symbol.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat)}");
                ...
                Log($"ConvertMethod finished: {symbol.Name}");
            }
            else
            {
                Log($"ConvertMethod cache hit: {symbol.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat)}");
            }
            return method;
        }
