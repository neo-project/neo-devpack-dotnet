...
namespace Neo.Compiler
{
    public class CompilationContext
    {
        ...
        private void ProcessMethod(SemanticModel model, IMethodSymbol symbol, bool export)
        {
            ...
            if (symbol.GetAttributesWithInherited().Any(p => p.AttributeClass?.Name == "MethodImplAttribute" && (MethodImplOptions)p.ConstructorArguments[0].Value == MethodImplOptions.AggressiveInlining))
                return;
