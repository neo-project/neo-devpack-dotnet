...
public class RateLimitingMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        var enableRateLimiting = _configuration.GetValue<bool>("R3EWebGUI:RateLimiting:EnableRateLimiting", true);
        var permitLimit = _configuration.GetValue<int>("R3EWebGUI:RateLimiting:PermitLimit", 100);
        var windowMinutes = _configuration.GetValue<int>("R3EWebGUI:RateLimiting:WindowMinutes", 1);
        ...
        if (endpoint.Contains("/api/webgui/deploy"))
        {
            permitLimit = 10; // Stricter limit for deployment
        }
        else if (endpoint.Contains("/plugin"))
        {
            permitLimit = 20; // Stricter limit for plugin operations
        }
        ...
        lock (_lock)
        {
            ...
            // Check rate limit
            if (_requestHistory[key].Count >= permitLimit)
            {
                _logger.LogWarning("Rate limit exceeded for {Key}", key);
                context.Response.StatusCode = 429; // Too Many Requests
                context.Response.Headers.Add("Retry-After", new StringValues("60"));
