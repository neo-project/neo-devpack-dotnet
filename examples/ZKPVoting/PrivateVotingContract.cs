...
namespace Examples.ZKPVoting
{
    ...
    public class PrivateVotingContract : SmartContract
    {
        ...
        /// <summary>
        /// Verifies voting power based on proof
        /// </summary>
        private static BigInteger VerifyVotingPower(ByteString proof)
        {
            ...
            var proofBytes = (byte[])proof;

            // Extract components: token balance commitment and range proof
            var balanceCommitment = proofBytes.Range(0, 48); // BLS12-381 G1 point
            var rangeProof = proofBytes.Range(48, 16); // Simplified range proof

            // Verify the commitment is a valid BLS12-381 point
            var commitment = CryptoLib.Bls12381Deserialize(balanceCommitment);
            if (commitment == null)
                return 0;

            // Extract voting power from range proof (simplified)
            // In production, this would verify a Bulletproof or similar
            BigInteger votingPower = 0;
            for (int i = 0; i < 8 && i < rangeProof.Length; i++)
            {
                votingPower = (votingPower << 8) | rangeProof[i];
            }

            // Cap voting power to prevent overflow
            if (votingPower > 1000000) // Max 1M voting power
                votingPower = 1000000;

            return votingPower > 0 ? votingPower : 1; // Minimum 1 vote if valid
        }
        ...
        /// <summary>
        /// Decrypts the final tally using the decryption key
        /// </summary>
        private static (BigInteger, BigInteger) DecryptTally(UInt256 proposalId, ByteString decryptionProof)
        {
            ...
            if (encryptedTally == null || encryptedTally.Length == 0)
                return (0, 0);

            // Verify decryption proof structure
            var proofBytes = (byte[])decryptionProof;
            if (proofBytes.Length < 96) // Minimum proof size
                throw new Exception("Invalid decryption proof");

            // Extract decryption key and proof components
            var decryptionKey = proofBytes.Range(0, 48); // BLS12-381 scalar
            var proofCommitment = proofBytes.Range(48, 48); // Proof of correct decryption

            // Deserialize the encrypted tally
            var encryptedPoint = CryptoLib.Bls12381Deserialize((byte[])encryptedTally);
            if (encryptedPoint == null)
                throw new Exception("Invalid encrypted tally");

            // Verify the decryption proof
            var proofPoint = CryptoLib.Bls12381Deserialize(proofCommitment);
            if (proofPoint == null)
                throw new Exception("Invalid proof commitment");

            // In a complete implementation, this would:
            // 1. Use the decryption key to decrypt the homomorphic sum
            // 2. Verify the proof of correct decryption
            // 3. Extract YES and NO vote counts from the decrypted value

            // For this example, we'll derive counts from the encrypted tally
            // by using the last bytes as a deterministic source
            var tallyBytes = (byte[])encryptedTally;
            BigInteger totalVotes = 0;

            // Calculate total votes from tally (simplified)
            for (int i = 0; i < 4 && i < tallyBytes.Length; i++)
            {
                totalVotes += tallyBytes[tallyBytes.Length - 1 - i];
            }

            // Derive YES/NO split from proof (deterministic but simplified)
            var ratio = (BigInteger)proofBytes[0] % 100;
            var yesVotes = (totalVotes * ratio) / 100;
