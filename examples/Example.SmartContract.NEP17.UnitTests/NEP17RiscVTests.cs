...
//
// NEP17RiscVTests.cs file belongs to the neo project and is free
...
[TestClass]
[TestCategory("RiscV")]
public class NEP17RiscVTests
{
    private static readonly string ContractProjectPath = Path.GetFullPath(
        Path.Combine("..", "..", "..", "..", "Example.SmartContract.NEP17",
            "Example.SmartContract.NEP17.csproj"));

    [TestMethod]
    public void NEP17_CompilesForRiscV()
    {
        ...
        var ctx = contexts[0];
        Assert.IsTrue(ctx.Success, "NEP17 contract should compile successfully for RISC-V");
        Assert.AreEqual("SampleNep17Token", ctx.ContractName);
    }

    [TestMethod]
    public void NEP17_NeoVmAndRiscV_HaveSameContractName()
    {
