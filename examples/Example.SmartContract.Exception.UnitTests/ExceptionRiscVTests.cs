...
//
// ExceptionRiscVTests.cs file belongs to the neo project and is free
...
[TestClass]
[TestCategory("RiscV")]
public class ExceptionRiscVTests
{
    private static readonly string ContractProjectPath = Path.GetFullPath(
        Path.Combine("..", "..", "..", "..", "Example.SmartContract.Exception",
            "Example.SmartContract.Exception.csproj"));

    [TestMethod]
    public void Exception_CompilesForRiscV()
    {
        ...
        var ctx = contexts[0];
        Assert.IsTrue(ctx.Success, "Exception contract should compile successfully for RISC-V");
        Assert.AreEqual("SampleException", ctx.ContractName);
    }

    [TestMethod]
    public void Exception_NeoVmAndRiscV_HaveSameContractName()
    {
