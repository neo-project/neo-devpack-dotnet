...
//
// ContractCallRiscVTests.cs file belongs to the neo project and is free
...
[TestClass]
[TestCategory("RiscV")]
public class ContractCallRiscVTests
{
    private static readonly string ContractProjectPath = Path.GetFullPath(
        Path.Combine("..", "..", "..", "..", "Example.SmartContract.ContractCall",
            "Example.SmartContract.ContractCall.csproj"));

    [TestMethod]
    public void ContractCall_CompilesForRiscV()
    {
        ...
        var ctx = contexts[0];
        Assert.IsTrue(ctx.Success, "ContractCall should compile successfully for RISC-V");
        Assert.AreEqual("SampleContractCall", ctx.ContractName);
    }

    [TestMethod]
    public void ContractCall_NeoVmAndRiscV_HaveSameContractName()
    {
