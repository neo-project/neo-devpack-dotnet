...
//
// OracleRiscVTests.cs file belongs to the neo project and is free
...
[TestClass]
[TestCategory("RiscV")]
public class OracleRiscVTests
{
    private static readonly string ContractProjectPath = Path.GetFullPath(
        Path.Combine("..", "..", "..", "..", "Example.SmartContract.Oracle",
            "Example.SmartContract.Oracle.csproj"));

    [TestMethod]
    public void Oracle_CompilesForRiscV()
    {
        ...
        var ctx = contexts[0];
        Assert.IsTrue(ctx.Success, "Oracle contract should compile successfully for RISC-V");
        Assert.AreEqual("SampleOracle", ctx.ContractName);
    }

    [TestMethod]
    public void Oracle_NeoVmAndRiscV_HaveSameContractName()
    {
