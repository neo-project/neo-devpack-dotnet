...
namespace Examples.ZKPTransaction
{
    ...
    public class PrivateTransactionContract : SmartContract
    {
        ...
        private static bool TransferToContract(UInt160 from, BigInteger amount)
        {
            ...
            // Call NEP-17 transfer (assuming GAS token for this example)
            // In production, you would configure the token contract address
            var gasContract = GAS.Hash;
            var contractAddress = Runtime.ExecutingScriptHash;

            // Transfer GAS from user to this contract
            object[] args = new object[] { from, contractAddress, amount, null };
            var result = Contract.Call(gasContract, "transfer", CallFlags.All, args);
        ...
        private static bool TransferFromContract(UInt160 to, BigInteger amount)
        {
            ...
            // Transfer GAS from this contract to user
            object[] args = new object[] { contractAddress, to, amount, null };
            var result = Contract.Call(gasContract, "transfer", CallFlags.All, args);
        ...
        private static void ChargeFee()
        {
            ...
            if (feeCollector != null && feeCollector.Length == 20)
            {
                var sender = Runtime.Transaction.Sender;
                var collector = (UInt160)feeCollector;

                // Transfer privacy fee from sender to fee collector
                var gasContract = GAS.Hash;
                object[] args = new object[] { sender, collector, PRIVACY_FEE, "Privacy operation fee" };
                Contract.Call(gasContract, "transfer", CallFlags.All, args);
            }
        }
