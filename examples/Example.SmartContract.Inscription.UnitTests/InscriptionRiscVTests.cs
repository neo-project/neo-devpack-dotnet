...
//
// InscriptionRiscVTests.cs file belongs to the neo project and is free
...
[TestClass]
[TestCategory("RiscV")]
public class InscriptionRiscVTests
{
    private static readonly string ContractProjectPath = Path.GetFullPath(
        Path.Combine("..", "..", "..", "..", "Example.SmartContract.Inscription",
            "Example.SmartContract.Inscription.csproj"));

    [TestMethod]
    public void Inscription_CompilesForRiscV()
    {
        ...
        var ctx = contexts[0];
        Assert.IsTrue(ctx.Success, "Inscription contract should compile successfully for RISC-V");
        Assert.AreEqual("SampleInscription", ctx.ContractName);
    }

    [TestMethod]
    public void Inscription_NeoVmAndRiscV_HaveSameContractName()
    {
