...
//
// NFTRiscVTests.cs file belongs to the neo project and is free
...
[TestClass]
[TestCategory("RiscV")]
public class NFTRiscVTests
{
    private static readonly string ContractProjectPath = Path.GetFullPath(
        Path.Combine("..", "..", "..", "..", "Example.SmartContract.NFT",
            "Example.SmartContract.NFT.csproj"));

    [TestMethod]
    [Ignore("NFT contract triggers pre-existing compiler bug: 'SyntaxTree is not part of the compilation'")]
    public void NFT_CompilesForRiscV()
    {
        ...
        var ctx = contexts[0];
        Assert.IsTrue(ctx.Success, "NFT contract should compile successfully for RISC-V");
        Assert.AreEqual("SampleLootNFT", ctx.ContractName);
    }

    [TestMethod]
    [Ignore("NFT contract triggers pre-existing compiler bug: 'SyntaxTree is not part of the compilation'")]
    public void NFT_NeoVmAndRiscV_HaveSameContractName()
    {
