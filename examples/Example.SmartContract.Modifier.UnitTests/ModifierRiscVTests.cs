...
//
// ModifierRiscVTests.cs file belongs to the neo project and is free
...
[TestClass]
[TestCategory("RiscV")]
public class ModifierRiscVTests
{
    private static readonly string ContractProjectPath = Path.GetFullPath(
        Path.Combine("..", "..", "..", "..", "Example.SmartContract.Modifier",
            "Example.SmartContract.Modifier.csproj"));

    [TestMethod]
    public void Modifier_CompilesForRiscV()
    {
        ...
        var ctx = contexts[0];
        Assert.IsTrue(ctx.Success, "Modifier contract should compile successfully for RISC-V");
        Assert.AreEqual("SampleModifier", ctx.ContractName);
    }

    [TestMethod]
    public void Modifier_NeoVmAndRiscV_HaveSameContractName()
    {
