...
//
// ZKPRiscVTests.cs file belongs to the neo project and is free
...
[TestClass]
[TestCategory("RiscV")]
public class ZKPRiscVTests
{
    private static readonly string ContractProjectPath = Path.GetFullPath(
        Path.Combine("..", "..", "..", "..", "Example.SmartContract.ZKP",
            "Example.SmartContract.ZKP.csproj"));

    [TestMethod]
    public void ZKP_CompilesForRiscV()
    {
        ...
        var ctx = contexts[0];
        Assert.IsTrue(ctx.Success, "ZKP contract should compile successfully for RISC-V");
        Assert.AreEqual("SampleZKP", ctx.ContractName);
    }

    [TestMethod]
    public void ZKP_NeoVmAndRiscV_HaveSameContractName()
    {
