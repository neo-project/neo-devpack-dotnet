...
//
// StorageRiscVTests.cs file belongs to the neo project and is free
...
[TestClass]
[TestCategory("RiscV")]
public class StorageRiscVTests
{
    private static readonly string ContractProjectPath = Path.GetFullPath(
        Path.Combine("..", "..", "..", "..", "Example.SmartContract.Storage",
            "Example.SmartContract.Storage.csproj"));

    [TestMethod]
    public void Storage_CompilesForRiscV()
    {
        ...
        var ctx = contexts[0];
        Assert.IsTrue(ctx.Success, "Storage contract should compile successfully for RISC-V");
        Assert.AreEqual("SampleStorage", ctx.ContractName);
    }

    [TestMethod]
    public void Storage_NeoVmAndRiscV_HaveSameContractName()
    {
