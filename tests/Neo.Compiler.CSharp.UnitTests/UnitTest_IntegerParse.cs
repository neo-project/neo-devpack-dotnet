...
namespace Neo.Compiler.CSharp.UnitTests
{
    ...
    public class UnitTest_IntegerParse : TestBase<Contract_IntegerParse>
    {
        ...
        public void SByteParse_Test()
        {
            ...
            Assert.AreEqual(1003108510, Engine.FeeConsumed.Value);
            Assert.AreEqual(new BigInteger(sbyte.MinValue), Contract.TestSbyteparse("-128"));
            Assert.AreEqual(1005141220, Engine.FeeConsumed.Value);

            //test backspace trip
            Assert.ThrowsException<TestException>(() => Contract.TestSbyteparse("20 "));
            Assert.AreEqual(1007173450, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestSbyteparse(" 20 "));
            Assert.AreEqual(1009205680, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestSbyteparse("128"));
            Assert.AreEqual(1011253690, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestSbyteparse("-129"));
            Assert.AreEqual(1013301700, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestSbyteparse(""));
            Assert.AreEqual(1015333930, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestSbyteparse("abc"));
            Assert.AreEqual(1017366160, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestSbyteparse("@"));
            Assert.AreEqual(1019398390, Engine.FeeConsumed.Value);
        }
        ...
        public void ULongParse_Test()
        {
            ...
            Assert.AreEqual(1003108600, Engine.FeeConsumed.Value);
            Assert.AreEqual(new BigInteger(ulong.MaxValue), Contract.TestUlongparse("18446744073709551615"));
            Assert.AreEqual(1005141400, Engine.FeeConsumed.Value);

            //test backspace trip
            Assert.ThrowsException<TestException>(() => Contract.TestUlongparse("20 "));
            Assert.AreEqual(1007173630, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestUlongparse(" 20 "));
            Assert.AreEqual(1009205860, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestUlongparse("-1"));
            Assert.AreEqual(1011253960, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestUlongparse("18446744073709551616"));
            Assert.AreEqual(1013302060, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestUlongparse(""));
            Assert.AreEqual(1015334290, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestUlongparse("abc"));
            Assert.AreEqual(1017366520, Engine.FeeConsumed.Value);
            Assert.ThrowsException<TestException>(() => Contract.TestUlongparse("@"));
            Assert.AreEqual(1019398750, Engine.FeeConsumed.Value);
        }
