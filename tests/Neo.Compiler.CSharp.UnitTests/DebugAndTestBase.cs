    ...
    where T : SmartContract.Testing.SmartContract, IContractInfo
{
    ...
    private static void TestSingleContractBasicBlockStartEnd(CompilationContext result)
    {
        try
        {
            try
            ...
            {
                Console.WriteLine($"Omitted: {result.ContractName}");
            }
        }
        catch
        {
    ...
    private static void TestSingleContractBasicBlockStartEnd(NefFile nef, ContractManifest manifest, JToken debugInfo)
    {
        var basicBlocks = new ContractInBasicBlocks(nef, manifest, debugInfo);

        // TODO: support CALLA and do not return

        var instructions = basicBlocks.GetScriptInstructions().ToList();
        var (_, _, jumpTargets) =
        ...
        foreach (var basicBlock in basicBlocks.basicBlocks)
        {
        ...
        var includedInstructions = new HashSet<VM.Instruction>();
        foreach (var basicBlock in basicBlocks.basicBlocks)
