    ...
    public class StorageTest
    {
        private static void Put(TestEngine testengine, string method, byte prefix, byte[] key, byte[] value)
        {
            ...
            Assert.AreEqual(1,
                ...
                .Count(a =>
                    a.Key.Key.SequenceEqual(Concat(new byte[] { prefix }, key)) &&
        ...
        private static byte[] Get(TestEngine testengine, string method, byte prefix, byte[] key)
        {
            ...
            Assert.AreEqual(1, testengine.Snapshot.GetChangeSet().Count(a => a.Key.Key.SequenceEqual(Concat(new byte[] { prefix }, key))));
            return data.ToArray();
        }
        ...
        private static void Delete(TestEngine testengine, string method, byte prefix, byte[] key)
        {
            ...
            Assert.AreEqual(0, testengine.Snapshot.GetChangeSet().Count(a => a.Key.Key.SequenceEqual(Concat(new byte[] { prefix }, key))));
        }
        ...
        public void Test_Byte()
        {
            ...
            Put(testengine, "testPutByte", 0x11, key, value);

            // Get

            testengine.Reset();
            var getVal = Get(testengine, "testGetByte", 0x11, key);
            CollectionAssert.AreEqual(value, getVal);

            // Delete

            testengine.Reset();
            Delete(testengine, "testDeleteByte", 0x11, key);
        }
