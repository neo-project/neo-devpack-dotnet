...
As described in [§8.3.5](8-types.md#835-simple-types), the simple types provided by C#, such as `int`, `long`, and `bool`, are, in fact, all struct types.

## 16.2 Struct declarations

### 16.2.1 General

A *struct_declaration* is a *type_declaration* ([§14.7](14-namespaces.md#147-type-declarations)) that declares a new struct:
...
A *struct_declaration* consists of an optional set of *attributes* ([§22](22-attributes.md#22-attributes)), followed by an optional set of *struct_modifier*s ([§16.2.2](16-structs.md#1622-struct-modifiers)), followed by an optional `ref` modifier ([§16.2.3](16-structs.md#1623-ref-modifier)), followed by an optional partial modifier ([§15.2.7](15-classes.md#1527-partial-declarations)), followed by the keyword `struct` and an *identifier* that names the struct, followed by an optional *type_parameter_list* specification ([§15.2.3](15-classes.md#1523-type-parameters)), followed by an optional *struct_interfaces* specification ([§16.2.5](16-structs.md#1625-struct-interfaces)), followed by an optional *type_parameter_constraints-clauses* specification ([§15.2.5](15-classes.md#1525-type-parameter-constraints)), followed by a *struct_body* ([§16.2.6](16-structs.md#1626-struct-body)), optionally followed by a semicolon.
...
Except for `readonly`, the modifiers of a struct declaration have the same meaning as those of a class declaration ([§15.2.2](15-classes.md#1522-class-modifiers)).
...
- None of its instance properties shall have a *set_accessor_declaration* ([§15.7.3](15-classes.md#1573-accessors)).
- It shall not declare any field-like events ([§15.8.2](15-classes.md#1582-field-like-events)).
...
> *Note*: A `ref struct` shall not use a `yield return` or `yield break` statement within an instance method, because the implicit `this` parameter cannot be used in those contexts. *end note*
...
The handling of interfaces on multiple parts of a partial struct declaration ([§15.2.7](15-classes.md#1527-partial-declarations)) are discussed further in [§15.2.4.3](15-classes.md#15243-interface-implementations).
...
Except for the differences noted in [§16.4](16-structs.md#164-class-and-struct-differences), the descriptions of class members provided in [§15.3](15-classes.md#153-class-members) through [§15.12](15-classes.md#1512-static-constructors) apply to struct members as well.
...
- Structs are value types ([§16.4.2](16-structs.md#1642-value-semantics)).
- All struct types implicitly inherit from the class `System.ValueType` ([§16.4.3](16-structs.md#1643-inheritance)).
- Assignment to a variable of a struct type creates a *copy* of the value being assigned ([§16.4.4](16-structs.md#1644-assignment)).
- The default value of a struct is the value produced by setting all fields to their default value ([§16.4.5](16-structs.md#1645-default-values)).
- Boxing and unboxing operations are used to convert between a struct type and certain reference types ([§16.4.6](16-structs.md#1646-boxing-and-unboxing)).
- The meaning of `this` is different within struct members ([§16.4.7](16-structs.md#1647-meaning-of-this)).
- Instance field declarations for a struct are not permitted to include variable initializers ([§16.4.8](16-structs.md#1648-field-initializers)).
- A struct is not permitted to declare a parameterless instance constructor ([§16.4.9](16-structs.md#1649-constructors)).
- A struct is not permitted to declare a finalizer.

### 16.4.2 Value semantics

Structs are value types ([§8.3](8-types.md#83-value-types)) and are said to have value semantics. Classes, on the other hand, are reference types ([§8.2](8-types.md#82-reference-types)) and are said to have reference semantics.
...
With classes, it is possible for two variables to reference the same object, and thus possible for operations on one variable to affect the object referenced by the other variable. With structs, the variables each have their own copy of the data (except in the case of `in`, `out` and `ref` parameter variables), and it is not possible for operations on one to affect the other. Furthermore, except when explicitly nullable ([§8.3.12](8-types.md#8312-nullable-value-types)), it is not possible for values of a struct type to be `null`.
...
The default value of a struct corresponds to the value returned by the default constructor of the struct ([§8.3.3](8-types.md#833-default-constructors)). Unlike a class, a struct is not permitted to declare a parameterless instance constructor. Instead, every struct implicitly has a parameterless instance constructor, which always returns the value that results from setting all fields to their default values.
...
Since structs are not reference types, these operations are implemented differently for struct types. When a value of a struct type is converted to certain reference types (as defined in [§10.2.9](10-conversions.md#1029-boxing-conversions)), a boxing operation takes place. Likewise, when a value of certain reference types (as defined in [§10.3.7](10-conversions.md#1037-unboxing-conversions)) is converted back to a struct type, an unboxing operation takes place. A key difference from the same operations on class types is that boxing and unboxing *copies* the struct value either into or out of the boxed instance.
...
For further details on boxing and unboxing, see [§10.2.9](10-conversions.md#1029-boxing-conversions) and [§10.3.7](10-conversions.md#1037-unboxing-conversions).

### 16.4.7 Meaning of this

The meaning of `this` in a struct differs from the meaning of `this` in a class, as described in [§12.8.13](12-expressions.md#12813-this-access). When a struct type overrides a virtual method inherited from `System.ValueType` (such as `Equals`, `GetHashCode`, or `ToString`), invocation of the virtual method through an instance of the struct type does not cause boxing to occur. This is true even when the struct is used as a type parameter and the invocation occurs through an instance of the type parameter type.
...
>     {
>         T x = new T();
>         Runtime.Log(x.ToString());
>         Runtime.Log(x.ToString());
>         Runtime.Log(x.ToString());
>     }
>
>     public static void Test() => Test<Counter>();
> }
> ```
>
...
Unlike a class, a struct is not permitted to declare a parameterless instance constructor. Instead, every struct implicitly has a parameterless instance constructor, which always returns the value that results from setting all value type fields to their default value and all reference type fields to `null` ([§8.3.3](8-types.md#833-default-constructors)). A struct can declare instance constructors having parameters.
...
The `this` parameter of a struct instance constructor corresponds to an `out` parameter of the struct type. As such, `this` shall be definitely assigned ([§9.4](9-variables.md#94-definite-assignment)) at every location where the constructor returns. Similarly, it cannot be read (even implicitly) in the constructor body before being definitely assigned.
...
> There is one exception to this, and that involves automatically implemented properties ([§15.7.4](15-classes.md#1574-automatically-implemented-properties)). The definite assignment rules ([§12.21.2](12-expressions.md#12212-simple-assignment)) specifically exempt assignment to an auto-property of a struct type within an instance constructor of that struct type: such an assignment is considered a definite assignment of the hidden backing field of the auto-property. Thus, the following is allowed:
>
...
> *Note*: The creation of default values ([§16.4.5](16-structs.md#1645-default-values)) of struct types does not trigger the static constructor. (An example of this is the initial value of elements in an array.) *end note*

### 16.4.11 Automatically implemented properties

Automatically implemented properties ([§15.7.4](15-classes.md#1574-automatically-implemented-properties)) use hidden backing fields, which are only accessible to the property accessors.
...
There are three different safe-context values, the same as the ref-safe-context values defined for reference variables ([§9.7.2](9-variables.md#972-ref-safe-contexts)): **declaration-block**, **function-member**, and **caller-context**. The safe-context of an expression constrains its use as follows:
...
The application of a user-defined operator is treated as a method invocation ([§16.4.12.6](16-structs.md#164126-method-and-property-invocation)).
