...
An ***enum type*** is a distinct value type ([§8.3](8-types.md#83-value-types)) that declares a set of named constants.
...
<!-- markdownlint-enable MD028 -->
> *Note*: C# allows a trailing comma in an *enum_body*, just like it allows one in an *array_initializer* ([§17.7](17-arrays.md#177-array-initializers)). *end note*
...
The modifiers of an enum declaration have the same meaning as those of a class declaration ([§15.2.2](15-classes.md#1522-class-modifiers)). However, the `abstract`, and `sealed`, and `static` modifiers are not permitted in an enum declaration. Enums cannot be abstract and do not permit derivation.
...
>     {
>         Runtime.Log(StringFromColor(Color.Red));
>         Runtime.Log(StringFromColor(Color.Green));
>         Runtime.Log(StringFromColor(Color.Blue));
>     }
>
...
The type `System.Enum` is the abstract base class of all enum types (this is distinct and different from the underlying type of the enum type), and the members inherited from `System.Enum` are available in any enum type. A boxing conversion ([§10.2.9](10-conversions.md#1029-boxing-conversions)) exists from any enum type to `System.Enum`, and an unboxing conversion ([§10.3.7](10-conversions.md#1037-unboxing-conversions)) exists from `System.Enum` to any enum type.

Note that `System.Enum` is not itself an *enum_type*. Rather, it is a *class_type* from which all *enum_type*s are derived. The type `System.Enum` inherits from the type `System.ValueType` ([§8.3.2](8-types.md#832-the-systemvaluetype-type)), which, in turn, inherits from type `object`. At run-time, a value of type `System.Enum` can be `null` or a reference to a boxed value of any enum type.

## 19.6 Enum values and operations

Each enum type defines a distinct type; an explicit enumeration conversion ([§10.3.3](10-conversions.md#1033-explicit-enumeration-conversions)) is required to convert between an enum type and an integral type, or between two enum types. The set of values of the enum type is the same as the set of values of the underlying type and is not restricted to the values of the named constants. Any value of the underlying type of an enum can be cast to the enum type, and is a distinct valid value of that enum type.

Enum members have the type of their containing enum type (except within other enum member initializers: see [§19.4](19-enums.md#194-enum-members)). The value of an enum member declared in enum type `E` with associated value `v` is `(E)v`.

The following operators can be used on values of enum types:

- `==`, `!=`, `<`, `>`, `<=`, `>=` ([§12.12.6](12-expressions.md#12126-enumeration-comparison-operators))
- binary `+` ([§12.10.5](12-expressions.md#12105-addition-operator))
- binary `-` ([§12.10.6](12-expressions.md#12106-subtraction-operator))
- `^`, `&`, `|` ([§12.13.3](12-expressions.md#12133-enumeration-logical-operators))
- `~` ([§12.9.5](12-expressions.md#1295-bitwise-complement-operator))
- `++`, `--` ([§12.8.15](12-expressions.md#12815-postfix-increment-and-decrement-operators) and [§12.9.6](12-expressions.md#1296-prefix-increment-and-decrement-operators))
