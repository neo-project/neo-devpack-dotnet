...
Attributes are defined through the declaration of attribute classes ([§22.2](22-attributes.md#222-attribute-classes)), which can have positional and named parameters ([§22.2.3](22-attributes.md#2223-positional-and-named-parameters)). Attributes are attached to entities in a C# program using attribute specifications ([§22.3](22-attributes.md#223-attribute-specification)), and can be retrieved at run-time as attribute instances ([§22.4](22-attributes.md#224-attribute-instances)).
...
The attribute `AttributeUsage` ([§22.5.2](22-attributes.md#2252-the-attributeusage-attribute)) is used to describe how an attribute class can be used.

`AttributeUsage` has a positional parameter ([§22.2.3](22-attributes.md#2223-positional-and-named-parameters)) that enables an attribute class to specify the kinds of program entities on which it can be used.
...
***Attribute specification*** is the application of a previously defined attribute to a program entity. An attribute is a piece of additional declarative information that is specified for a program entity. Attributes can be specified at global scope (to specify attributes on the containing assembly or module) and for *type_declaration*s ([§14.7](14-namespaces.md#147-type-declarations)), *class_member_declaration*s ([§15.3](15-classes.md#153-class-members)), *interface_member_declaration*s ([§18.4](18-interfaces.md#184-interface-members)), *struct_member_declaration*s ([§16.3](16-structs.md#163-struct-members)), *enum_member_declaration*s ([§19.2](19-enums.md#192-enum-declarations)), *accessor_declaration*s ([§15.7.3](15-classes.md#1573-accessors)), *event_accessor_declaration*s ([§15.8](15-classes.md#158-events)), elements of *formal_parameter_list*s ([§15.6.2](15-classes.md#1562-method-parameters)), and elements of *type_parameter_list*s ([§15.2.3](15-classes.md#1523-type-parameters)).
...
> *Note*: For convenience, a trailing comma is allowed in a *global_attribute_section* and an *attribute_section*, just as one is allowed in an *array_initializer* ([§17.7](17-arrays.md#177-array-initializers)). *end note*
...
- If the right-most identifier of the *attribute_name* is a verbatim identifier ([§6.4.3](6-lexical-structure.md#643-identifiers)), then the *attribute_name* is resolved as a *type_name* ([§7.8](7-basic-concepts.md#78-namespace-and-type-names)). If the result is not a type derived from `System.Attribute`, a compile-time error occurs.
- Otherwise,
  - The *attribute_name* is resolved as a *type_name* ([§7.8](7-basic-concepts.md#78-namespace-and-type-names)) except any errors are suppressed. If this resolution is successful and results in a type derived from `System.Attribute` then the type is the result of this step.
  - The characters `Attribute` are appended to the right-most identifier in the *attribute_name* and the resulting string of tokens is resolved as a *type_name* ([§7.8](7-basic-concepts.md#78-namespace-and-type-names)) except any errors are suppressed. If this resolution is successful and results in a type derived from `System.Attribute` then the type is the result of this step.
...
> *Example*: If an attribute class is found both with and without this suffix, an ambiguity is present, and a compile-time error results. If the *attribute_name* is spelled such that its right-most *identifier* is a verbatim identifier ([§6.4.3](6-lexical-structure.md#643-identifiers)), then only an attribute without a suffix is matched, thus enabling such an ambiguity to be resolved. The example
>
...
Using the terms defined in [§22.4.2](22-attributes.md#2242-compilation-of-an-attribute), the attribute instance represented by `T`, `C`, `P`, and `N`, and associated with `E` can be retrieved at run-time from the assembly `A` using the following steps:
...
> *Example*: In an implementation of the CLI, the `Help` attribute instances in the assembly created by compiling the example program in [§22.2.3](22-attributes.md#2223-positional-and-named-parameters) can be retrieved with the following program:
>
...
- `System.AttributeUsageAttribute` ([§22.5.2](22-attributes.md#2252-the-attributeusage-attribute)), which is used to describe the ways in which an attribute class can be used.
- `System.Diagnostics.ConditionalAttribute` ([§22.5.3](22-attributes.md#2253-the-conditional-attribute)), is a multi-use attribute class which is used to define conditional methods and conditional attribute classes. This attribute indicates a condition by testing a conditional compilation symbol.
- `System.ObsoleteAttribute` ([§22.5.4](22-attributes.md#2254-the-obsolete-attribute)), which is used to mark a member as obsolete.
- `System.Runtime.CompilerServices.CallerLineNumberAttribute` ([§22.5.6.2](22-attributes.md#22562-the-callerlinenumber-attribute)), `System.Runtime.CompilerServices.CallerFilePathAttribute` ([§22.5.6.3](22-attributes.md#22563-the-callerfilepath-attribute)), and `System.Runtime.CompilerServices.CallerMemberNameAttribute` ([§22.5.6.4](22-attributes.md#22564-the-callermembername-attribute)), which are used to supply information about the calling context to optional parameters.
...
> *Note*: For an example of using this attribute, see [§22.2.2](22-attributes.md#2222-attribute-usage). *end note*
...
An attribute class ([§22.2](22-attributes.md#222-attribute-classes)) decorated with one or more `Conditional` attributes is a conditional attribute class. A conditional attribute class is thus associated with the conditional compilation symbols declared in its `Conditional` attributes.
...
Attribute specifications ([§22.3](22-attributes.md#223-attribute-specification)) of a conditional attribute are included if one or more of its associated conditional compilation symbols is defined at the point of specification, otherwise the attribute specification is omitted.
...
> )
> {
>     Runtime.Log((line < 0) ? "No line" : "Line "+ line);
>     Runtime.Log((path == null) ? "No file path" : path);
>     Runtime.Log((name == null) ? "No member name" : name);
> }
> ```
>
...
The attribute `System.Runtime.CompilerServices.CallerLineNumberAttribute` is allowed on optional parameters when there is a standard implicit conversion ([§10.4.2](10-conversions.md#1042-standard-implicit-conversions)) from the constant value `int.MaxValue` to the parameter’s type. This ensures that any non-negative line number up to that value can be passed without error.
...
The line number may be affected by `#line` directives ([§6.5.8](6-lexical-structure.md#658-line-directives)).

#### 22.5.6.3 The CallerFilePath attribute

The attribute `System.Runtime.CompilerServices.CallerFilePathAttribute` is allowed on optional parameters when there is a standard implicit conversion ([§10.4.2](10-conversions.md#1042-standard-implicit-conversions)) from `string` to the parameter’s type.
...
The file path may be affected by `#line` directives ([§6.5.8](6-lexical-structure.md#658-line-directives)).

#### 22.5.6.4 The CallerMemberName attribute

The attribute `System.Runtime.CompilerServices.CallerMemberNameAttribute` is allowed on optional parameters when there is a standard implicit conversion ([§10.4.2](10-conversions.md#1042-standard-implicit-conversions)) from `string` to the parameter’s type.
