...
A C# ***program*** consists of one or more source files, known formally as ***compilation units*** ([§14.2](14-namespaces.md#142-compilation-units)). Although a compilation unit might have a one-to-one correspondence with a file in a file system, such correspondence is not required.
...
> *Note*: The C# lexical grammar ([§6.2.3](6-lexical-structure.md#623-lexical-grammar)) and syntactic grammar ([§6.2.4](6-lexical-structure.md#624-syntactic-grammar)) are not in exact correspondence with the ANTLR division into lexical and parser grammers. This small mismatch means that some ANTLR parser rules are used when specifying the C# lexical grammar. *end note*

### 6.2.3 Lexical grammar

The lexical grammar of C# is presented in [§6.3](6-lexical-structure.md#63-lexical-analysis), [§6.4](6-lexical-structure.md#64-tokens), and [§6.5](6-lexical-structure.md#65-pre-processing-directives). The terminal symbols of the lexical grammar are the characters of the Unicode character set, and the lexical grammar specifies how characters are combined to form tokens ([§6.4](6-lexical-structure.md#64-tokens)), white space ([§6.3.4](6-lexical-structure.md#634-white-space)), comments ([§6.3.3](6-lexical-structure.md#633-comments)), and pre-processing directives ([§6.5](6-lexical-structure.md#65-pre-processing-directives)).
...
Every compilation unit in a C# program shall conform to the *input* production of the lexical grammar ([§6.3.1](6-lexical-structure.md#631-general)).

### 6.2.4 Syntactic grammar

The syntactic grammar of C# is presented in the clauses, subclauses, and annexes that follow this subclause. The terminal symbols of the syntactic grammar are the tokens defined explicitly by the lexical grammar and implicitly by literal strings in the grammar itself ([§6.2.3](6-lexical-structure.md#623-lexical-grammar)). The syntactic grammar specifies how tokens are combined to form C# programs.

Every compilation unit in a C# program shall conform to the *compilation_unit* production ([§14.2](14-namespaces.md#142-compilation-units)) of the syntactic grammar.

### 6.2.5 Grammar ambiguities

The productions for *simple_name* ([§12.8.4](12-expressions.md#1284-simple-names)) and *member_access* ([§12.8.7](12-expressions.md#1287-member-access)) can give rise to ambiguities in the grammar for expressions.
...
If a sequence of tokens can be parsed (in context) as a *simple_name* ([§12.8.4](12-expressions.md#1284-simple-names)), *member_access* ([§12.8.7](12-expressions.md#1287-member-access)), or *pointer_member_access* ([§23.6.3](unsafe-code.md#2363-pointer-member-access)) ending with a *type_argument_list* ([§8.4.2](8-types.md#842-type-arguments)), the token immediately following the closing `>` token is examined, to see if it is
...
> *Note*: These rules are not applied when parsing a *type_argument_list* in a *namespace_or_type_name* ([§7.8](7-basic-concepts.md#78-namespace-and-type-names)). *end note*
<!-- markdownlint-disable MD028 -->
...
When recognising a *relational_expression* ([§12.12.1](12-expressions.md#12121-general)) if both the “*relational_expression* `is` *type*” and “*relational_expression* `is` *constant_pattern*” alternatives are applicable, and *type* resolves to an accessible type, then the “*relational_expression* `is` *type*” alternative shall be chosen.
...
Five basic elements make up the lexical structure of a C# compilation unit: Line terminators ([§6.3.2](6-lexical-structure.md#632-line-terminators)), white space ([§6.3.4](6-lexical-structure.md#634-white-space)), comments ([§6.3.3](6-lexical-structure.md#633-comments)), tokens ([§6.4](6-lexical-structure.md#64-tokens)), and pre-processing directives ([§6.5](6-lexical-structure.md#65-pre-processing-directives)). Of these basic elements, only tokens are significant in the syntactic grammar of a C# program ([§6.2.4](6-lexical-structure.md#624-syntactic-grammar)).
...
> *Note*: The additional carriage-return allows a program to end in a *PP_Directive* ([§6.5](6-lexical-structure.md#65-pre-processing-directives)) that does not have a terminating *New_Line*. *end note*
...
> // This program writes "hello, world" to the console
> //
> class Hello : SmartContract.Framework.SmartContract // any name will do for this class
> {
>     public static void Test() // this method must be public to be called by transaction
>     {
>         Runtime.Log("hello, world");
>     }
> }
...
A Unicode escape sequence represents a Unicode code point. Unicode escape sequences are processed in identifiers ([§6.4.3](6-lexical-structure.md#643-identifiers)), character literals ([§6.4.5.5](6-lexical-structure.md#6455-character-literals)), regular string literals ([§6.4.5.6](6-lexical-structure.md#6456-string-literals)), and interpolated regular string expressions ([§12.8.3](12-expressions.md#1283-interpolated-string-expressions)). A Unicode escape sequence is not processed in any other location (for example, to form an operator, punctuator, or keyword).
...
> - Fragment *Escaped_Identifier* includes escaped keywords and contextual keywords as they are part of the longer token starting with an `@` and lexical processing always forms the longest possible lexical element ([§6.3.1](6-lexical-structure.md#631-general)).
...
- It can denote a discard ([§9.2.9.1](9-variables.md#9291-discards)).
...
> *Note*: The rules *keyword* and *contextual_keyword* are parser rules as they do not introduce new token kinds. All keywords and contextual keywords are defined by implicit lexical rules as they occur as literal strings in the grammar ([§6.2.3](6-lexical-structure.md#623-lexical-grammar)). *end note*

In most cases, the syntactic location of contextual keywords is such that they can never be confused with ordinary identifier usage. For example, within a property declaration, the `get` and `set` identifiers have special meaning ([§15.7.3](15-classes.md#1573-accessors)). An identifier other than `get` or `set` is never permitted in these locations, so this use does not conflict with a use of these words as identifiers.

In certain cases the grammar is not enough to distinguish contextual keyword usage from identifiers. In all such cases it will be specified how to disambiguate between the two. For example, the contextual keyword `var` in implicitly typed local variable declarations ([§13.6.2](13-statements.md#1362-local-variable-declarations)) might conflict with a declared type called `var`, in which case the declared name takes precedence over the use of the identifier as a contextual keyword.
...
A ***literal*** ([§12.8.2](12-expressions.md#1282-literals)) is a source-code representation of a value.
...
A Unicode escape sequence ([§6.4.2](6-lexical-structure.md#642-unicode-character-escape-sequences)) in a character literal shall be in the range `U+0000` to `U+FFFF`.
...
Each string literal does not necessarily result in a new string instance. When two or more string literals that are equivalent according to the string equality operator ([§12.12.8](12-expressions.md#12128-string-equality-operators)), appear in the same assembly, these string literals refer to the same string instance.
...
A *null_literal* represents a `null` value. It does not have a type, but can be converted to any reference type or nullable value type through a null literal conversion ([§10.2.7](10-conversions.md#1027-null-literal-conversions)).
...
- `#define` and `#undef`, which are used to define and undefine, respectively, conditional compilation symbols ([§6.5.4](6-lexical-structure.md#654-definition-directives)).
- `#if`, `#elif`, `#else`, and `#endif`, which are used to skip conditionally sections of source code ([§6.5.5](6-lexical-structure.md#655-conditional-compilation-directives)).
- `#line`, which is used to control line numbers emitted for errors and warnings ([§6.5.8](6-lexical-structure.md#658-line-directives)).
- `#error`, which is used to issue errors ([§6.5.6](6-lexical-structure.md#656-diagnostic-directives)).
- `#region` and `#endregion`, which are used to explicitly mark sections of source code ([§6.5.7](6-lexical-structure.md#657-region-directives)).
- `#pragma`, which is used to specify optional contextual information to a compiler ([§6.5.10](6-lexical-structure.md#6510-pragma-directives)).
- `#nullable`, which is used to specify the nullable context ([§6.5.9](6-lexical-structure.md#659-nullable-directive)).
...
The conditional compilation functionality provided by the `#if`, `#elif`, `#else`, and `#endif` directives is controlled through pre-processing expressions ([§6.5.3](6-lexical-structure.md#653-pre-processing-expressions)) and conditional compilation symbols.
...
Evaluation of a pre-processing expression always yields a Boolean value. The rules of evaluation for a pre-processing expression are the same as those for a constant expression ([§12.23](12-expressions.md#1223-constant-expressions)), except that the only user-defined entities that can be referenced are conditional compilation symbols.
...
Any `#define` and `#undef` directives in a compilation unit shall occur before the first *token* ([§6.4](6-lexical-structure.md#64-tokens)) in the compilation unit; otherwise a compile-time error occurs. In intuitive terms, `#define` and `#undef` directives shall precede any “real code” in the compilation unit.
...
Line directives may be used to alter the line numbers and compilation unit names that are reported by the compiler in output such as warnings and errors. These values are also used by caller-info attributes ([§22.5.6](22-attributes.md#2256-caller-info-attributes)).
...
> string y = "";
> #nullable enable
> Runtime.Log(x.Length); // Warning
> Runtime.Log(y.Length);
> ```
>
