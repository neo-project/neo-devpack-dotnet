...
Status: supported
Scope: class
Notes: Required members can be declared and the compiler enforces definite assignment.
```csharp
...
Status: supported
Scope: class
Notes: Generic attribute types compile without issue.
```csharp
...
Status: supported
Scope: class
Notes: Static abstract interface members that model generic math compile successfully.
```csharp
...
Status: supported
Scope: class
Notes: Method groups now convert more reliably to delegates and compile without additional hints.
```csharp
...
Status: supported
Scope: method
Notes: Pattern matching `ReadOnlySpan<char>` against constant strings compiles successfully.
```csharp
...
Status: supported
Scope: class
Notes: `ref` fields and scoped variables in `ref struct` types compile successfully.
```csharp
