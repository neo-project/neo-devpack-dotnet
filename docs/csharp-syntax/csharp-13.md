...
Status: supported
Scope: class
Notes: The compiler already accepts `params` applied to `ReadOnlySpan<T>` and other collection-based parameters.
```csharp
...
Status: unsupported
Scope: method
Notes: Using the new `System.Threading.Lock` synchronization primitive requires the C# 13 compiler pipeline.
```csharp
...
Status: supported
Scope: method
Notes: Using `\e` compiles successfully with the current compiler.
```csharp
char escape = '\e';
```
...
Status: supported
Scope: class
Notes: These improvements affect overload resolution semantics without introducing new syntax.
```csharp
...
Status: unsupported
Scope: method
Notes: Using the `^` operator inside an object initializer is a C# 13 addition.
```csharp
...
Status: unsupported
Scope: method
Notes: Declaring `ref` locals inside `async` methods is newly permitted in C# 13.
```csharp
...
Status: supported
Scope: class
Notes: Generic constraints using `allows ref struct` compile without error.
```csharp
...
Status: supported
Scope: class
Notes: `ref struct` types implementing interfaces compile without error.
```csharp
...
Status: supported
Scope: class
Notes: Partial properties compile, indicating the rule is already satisfied.
```csharp
...
Status: supported
Scope: class
Notes: Applying `OverloadResolutionPriority` compiles successfully.
```csharp
...
Status: unsupported
Scope: class
Notes: The preview `field` keyword exposes auto-property backing fields when enabled.
```csharp
...
Status: unsupported
Scope: file
Notes: Extension type declarations require preview compiler support.
```csharp
