...
Status: supported
Scope: method
Notes: Switch expressions compile and can be used inside contract methods.
```csharp
int value = 3;
...
Status: unsupported
Scope: method
Notes: The ^ and .. operators are not recognized by the Neo compiler.
```csharp
int[] values = { 1, 2, 3, 4 };
int last = values[^1];
...
Status: supported
Scope: method
Notes: The ??= operator compiles and mutates the target as expected.
```csharp
string? text = null;
text ??= "neo";
```
...
Status: supported
Scope: method
Notes: Using declarations for IDisposable instances compile and scope the resource to the current block.
```csharp
...
Status: supported
Scope: class
Notes: Nullable annotations compile and respect the nullable context enabled by the compiler options.
```csharp
public class Holder
{
...
Status: unsupported
Scope: class
Notes: Await foreach and IAsyncEnumerable members are rejected by the Neo compiler.
```csharp
